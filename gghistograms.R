...
for(var in c('displacement','movement','ratio')){
  ...
  df=data.frame(n=grz,brks,activity="grazing")
  df=rbind(df,data.frame(n=rst,brks,activity="resting"))
  df=rbind(df,data.frame(n=wlk,brks,activity="walking"))
  plt=ggplot(df,aes(x=brks,y=n,color=activity,linetype=activity))+labs(title=paste(lokname,var))
  plt=plt+xlab(unit)
  if((var=='displacement' && rtyp=='d') || (var=='movement' && rtyp=='t'))
    plt=plt+geom_vline(xintercept=rspeed)
  if((var=='displacement' && wtyp=='d') || (var=='movement' && wtyp=='t'))
    plt=plt+geom_vline(xintercept=wspeed)
  if(var=='ratio'){
    if(rratio>0) plt=plt+geom_vline(xintercept=rratio)
    if(wratio<1) plt=plt+geom_vline(xintercept=wratio)
  }   
# mtext(paste(letters[l],")",sep=''),line=2,at=c(-0.15*max(brks)),cex=1.5)
  l=l+1
  plt=plt+geom_step()+theme_bw()
  plt=plt+scale_colour_manual(values=c('black','#555555','#999999'))
  print(plt)
  ggsave(file)
}
